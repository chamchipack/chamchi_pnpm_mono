...
>(
  ...
  return useSWR<ClientResponse<SchemaTransform<T, Op, F>>>(
    `${process.env.NEXT_PUBLIC_API_ADDRESS}/api/app/coupon/${userId}?${qs.stringify(params)}`,
  );
};
...
>(
  ...
  return useSWR<ClientResponse<SchemaTransform<T, Op, F>>>(
    `${process.env.NEXT_PUBLIC_API_ADDRESS}/api/app/coupon/available/${userId}/${sellerId}`,
  );
};

export const useCouponsInfinite = <
  T extends DataStructureKey,
  Op extends 'none' | 'Omit' | 'Pick' | 'Partial' | 'Readonly' = 'none',
  F extends keyof StructuredDataSchemas[T] = never,
>(
  ...
  return useSWRInfinite<ClientResponse<SchemaTransform<T, Op, F>>>(
    (pageIndex, previousPageData) => {
      ...
      if (
        ...
        return null;
      return `${process.env.NEXT_PUBLIC_API_ADDRESS}/api/app/coupon/${userId}?${qs.stringify({ page: pageIndex + 1, limit })}`;
    },
    {
...
export async function updateBookmark(
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_ADDRESS}/api/app/bookmark`,
      {
        method: 'POST',
        headers: {
