...
>(
  ...
  return useSWR<ClientResponse<SchemaTransform<T, Op, F>>>(
    `${process.env.NEXT_PUBLIC_API_ADDRESS}/api/admin/announcement?${qs.stringify(params)}`,
  );
};
  ...
  return useSWRInfinite<ClientResponse<SchemaTransform<T, Op, F>>>(
    (pageIndex, previousPageData) => {
      ...
      if (
        ...
        return null;
      return `${process.env.NEXT_PUBLIC_API_ADDRESS}/api/admin/announcement?${qs.stringify({ page: pageIndex + 1, limit })}`;
    },
    {
