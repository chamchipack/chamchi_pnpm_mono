...
export function useLocation(
  ...
  const handleSaveLocation = () => {
    ...
    try {
      ...
      const isDuplicate = list.some((item) => item.address === dataset.address);
      ...
      setLocationCookie(currentPosition?.latitude, currentPosition?.longitude);

      handleStorage({ data });
      setFindMode(false);
      if (handleTerminate) handleTerminate();
      router.refresh();
    } catch {}
  };
