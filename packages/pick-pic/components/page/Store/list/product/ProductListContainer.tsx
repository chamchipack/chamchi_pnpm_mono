...
import { useProductSearchInfinite } from '@/api/client/product';
import CSR_ProductImage from '@/components/page/Populars/images/CSR_ProductImage';
...
export default function ProductListContainer({ params }: Props) {
  ...
  const date = params?.date
    ? { date: dayjs(params?.date).format('YYYY-MM-DDTHH:mm:ss') }
    : {};

  const limit = 15;
  const { data, size, setSize, isValidating } = useProductSearchInfinite<
  ...
  const handleRouter = (alias: string, productId: string) => {
    ...
    if (typeof window !== 'undefined' && (window as any).ReactNativeWebView) {
      (window as any).ReactNativeWebView?.postMessage(
        JSON.stringify({ type: 'ALIAS', data: `${alias}/${productId}` }),
      );
        ...
        <div className="grid grid-cols-3 gap-0.5 w-full">
          {Array.from({ length: 10 }).map((_, index) => (
            <div key={index} className="mb-px mx-0.5">
              <div className="w-full h-[200px] bg-gray-200 animate-pulse rounded-sm" />
            </div>
          ))}
        </div>
      ) : (
          ...
          {items?.length ? (
            <div className="grid grid-cols-3 w-full">
              {items.map((item, index) => (
                ...
                // <div
                //   key={item._id}
                //   className={`relative w-full h-[200px] overflow-hidden mb-px border-b border-white ${
